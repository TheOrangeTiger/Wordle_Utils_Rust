...
use wasm_bindgen::prelude::*; //For release only
// Current winrate 97.12% :)))
const WORDS: &str = include_str!("../wordle-words.txt");
// const WORDS: &str = include_str!("../test-words.txt"); // for tests only
#[wasm_bindgen]
...
pub fn suggest_guess(
    ...
    let words = filter_words(
        &bannedlets.chars().collect(),
        &bannedatlets,
        &mustinclets.chars().collect(),
        &incatlets,
        &wordlist,
    );
    if words.len() == 1 {
...
pub fn solve_wordle_binary() {
    ...
    let mut bannedatlets: Vec<String> = vec![
        String::new(),
        String::new(),
        String::new(),
        String::new(),
    ...
    for _ in 0..6 {
        ...
        (bannedlets, bannedatlets, mustinclets, incatlets) = lists_from_input(
            &guess,
            bannedlets,
            bannedatlets,
            mustinclets,
...
fn get_user_input(
    ...
    loop {
        ...
        if input.len() == 8 && input.starts_with("or") {
            ...
            if wordlist.contains(&new_guess) {
                guess = new_guess;
            }
            ...
            if !ng && guess == "-----".to_string() {
                guess = old_guess.clone()
            }
        } else if guess != "-----".to_string()
            && input.len() == 5
            && input.chars().all(|x| ['g', 'n', 'y'].contains(&x))
        {
            return (input, guess, ng);
        }
    }
}
...
fn filter_words(
    bannedlets: &Vec<char>,
    bannedatlets: &Vec<String>,
    mustinclets: &Vec<char>,
    incatlets: &str,
    wordlist: &Vec<String>,
) -> Vec<String> {
    let mut poswords: Vec<String> = vec![];
    'word: for word in wordlist {
        for lett in bannedlets {
            if word.contains(*lett) {
                continue 'word;
            }
        }
        for lett in mustinclets {
            if !word.contains(*lett) {
                continue 'word;
            }
        }
...
fn word_chooser(wordlist: &Vec<String>, narrowed_down_list: Vec<String>) -> String {
    ...
    if narrowed_down_list.len() <= 2 && not_fw_mode {
        return narrowed_down_list.get(0).unwrap().clone();
    }
    ...
    if not_fw_mode {
        for (_, v) in &mut wordcount {
            if *v >= narrowed_down_list.len() as i32 {
                *v = 0;
            }
        }
    }
    ...
    for word in wordlist {
        ...
        for char in chars {
            *wordscore.entry(word).or_insert(0) += wordcount.get(&char).unwrap_or(&0);
        }
    }
    let mut wordscore: Vec<(&String, i32)> = wordscore.into_iter().collect();
    wordscore.sort_by(|a, b| b.1.cmp(&a.1).then_with(|| a.0.cmp(b.0)));
    if wordscore[0].1 > 0 {
        return wordscore[0].0.to_string();
    }
    narrowed_down_list.get(0).unwrap().clone()
}
#[wasm_bindgen]
pub fn get_words() -> Vec<String> {
    WORDS.to_string().lines().map(|x| x.to_string()).collect()
}
#[wasm_bindgen]
#[allow(dead_code)]
...
#[wasm_bindgen]
impl ListJsOutput {
    pub fn get_bannedlets(&self) -> String {
        self.bannedlets.clone()
    }
    pub fn get_bannedatlets(&self) -> Vec<String> {
        self.bannedatlets.clone()
    }
    pub fn get_mustinclets(&self) -> String {
        self.mustinclets.clone()
    }
    pub fn get_incatlets(&self) -> String {
        self.incatlets.clone()
    }
}
#[wasm_bindgen]
pub fn lists_from_input_js(
    guess: &str,
    bannedlets: String,
    bannedatlets: Vec<String>,
    mustinclets: String,
    ...
    let (bannedlets, bannedatlets, mustinclets, incatlets) = lists_from_input(
        guess,
        bannedlets.chars().collect(),
        bannedatlets,
        mustinclets.chars().collect(),
    ...
    ListJsOutput {
        bannedlets: bannedlets.iter().collect(),
        bannedatlets,
        mustinclets: mustinclets.iter().collect(),
        incatlets,
    }
}
fn lists_from_input(
    guess: &str,
    mut bannedlets: Vec<char>,
    mut bannedatlets: Vec<String>,
    mut mustinclets: Vec<char>,
    mut incatlets: String,
    input: String,
) -> (Vec<char>, Vec<String>, Vec<char>, String) {
    ...
    for char in &bannedlets {
        if incatlets.contains(*char) || mustinclets.iter().any(|c| c == char) {
            continue;
        }
        temp.push(*char);
    }
